 ...
 *
 * Merges applied this session:
 *   • ExitDialog absorbed as showExitDialog() static helper — it was a 35-line
 *     modal with no state; a static method here is identical and removes a file.
 *
 * New additions (no features removed):
 *   • EmergencyCardUI tab added inside "Get Help" (3rd inner tab)
 *   • ContentService used for quotes/prompts (QuoteService/PromptService
 *     remain as thin delegates so no call sites break)
 *
 * Nav: Home · Wellness Tools · Journal · Get Help · Profile · Settings + Logout
 */
...
public class MainDashboardUI {
    ...
    /**
     * Formerly ExitDialog.java — a 35-line file with no state.
     * A static method here is simpler and removes a class file.
     * All behaviour (modal, undecorated, warm styling) is preserved exactly.
     */
    public static void showExitDialog(Stage owner) {
        ...
        layout.setStyle(
            "-fx-background-color: #fff3e0; -fx-border-color: #ffb74d; " +
            "-fx-border-width: 3; -fx-background-radius: 15; -fx-border-radius: 15;"
        );
    ...
    private void buildSidebar() {
        ...
        sidebar.getChildren().addAll(
            appTitle, btnHome, btnWellness, btnJournal,
            btnHelp, btnProfile, btnSettings, spacer, btnLogout
        );
    ...
    private void setupNavigation() {
        ...
        // Window close button uses exit dialog instead of abrupt quit
    ...
    /** Wellness Tools: Breathe + Calming Station */
    private Node createWellnessView() {
        TabPane tabs = new TabPane();
        tabs.setTabClosingPolicy(TabPane.TabClosingPolicy.UNAVAILABLE);
        tabs.setStyle("-fx-background-color: transparent;");
        tabs.getTabs().addAll(
            new Tab("🌬  Breathe",          BreatheUI.getBreatheNode()),
            new Tab("🧘  Calming Station",   CalmingStationUI.getCalmingNode(darkMode))
        );
        return tabs;
    }

    /** Journal: Journal entries + Sleep Tracker */
    private Node createJournalView() {
        TabPane tabs = new TabPane();
        tabs.setTabClosingPolicy(TabPane.TabClosingPolicy.UNAVAILABLE);
        JournalUI journal = new JournalUI(stage, journalService, username, darkMode, globalFontSize);
        tabs.getTabs().addAll(
            new Tab("📓  Journal",          journal.getContentNode()),
            new Tab("🌙  Sleep Tracker",     SleepTrackerUI.getSleepNode(username, darkMode))
        );
        return tabs;
    }

    /**
     * Get Help: Crisis Dashboard + Connect to 311 + Emergency Card.
     * EmergencyCardUI is the third tab — works fully offline, covers all of Canada
     * including rural and remote users.
     */
    private Node createGetHelpView() {
        ...
        tabs.getTabs().addAll(
            ...
            new Tab("📞  Connect to 311",      Connect311UI.get311Node(darkMode)),
            new Tab("🪪  Emergency Card",      EmergencyCardUI.getCardNode(username, darkMode))
        );
        return tabs;
    }
    ...
    private VBox createHomeView() {
        ...
        // Uses ContentService directly; QuoteService.getRandomQuote() also still works
        Label quoteLabel = new Label("\"" + ContentService.getQuote(preferredQuoteCategory) + "\"");
    ...
    private Node createSettingsView() {
        ...
        breathBox.setOnAction(e -> { breathingPattern = breathBox.getValue(); syncSettingsToServices(); });
        ...
        quoteBox.setOnAction(e -> { preferredQuoteCategory = quoteBox.getValue(); syncSettingsToServices(); });
        ...
        pathField.textProperty().addListener((obs, o, n) -> { modelPath = n; syncSettingsToServices(); });
    ...
    private void applyTheme() {
        if (darkMode) {
            ...
            sidebar.setStyle("-fx-background-color: #0a240a; -fx-border-color: #1b4d1b; -fx-border-width: 0 1 0 0;");
        } else {
